...
public class DocumentViewModule extends ReactContextBaseJavaModule implements ActivityEventListener {
    ...
    @ReactMethod
    public void toText(final int tag, final int number, final Promise promise) {
        getReactApplicationContext().runOnUiQueueThread(new Runnable() {
            @Override
            public void run() {
                try {
                    String txt = mDocumentViewInstance.toText(tag, number);
                    promise.resolve(txt);
                } catch (Exception ex) {
                    promise.reject(ex);
                }
    ...
    @ReactMethod
    public void pageLabel(final int tag, final ReadableArray item) {
        getReactApplicationContext().runOnUiQueueThread(new Runnable() {
            @Override
            public void run() {
                try {
                    mDocumentViewInstance.pageLabel(tag, item);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
